...
export default function Homme({ allProducts }) {
  ...
  <div className="flex flex-wrap mt-2">
    {["Noir", "Blanc", "Rose", "Maron", "Blue"].map((color) => (
      <div key={color} className="mr-2 mb-2">
        <input
          type="checkbox"
          id={color}
        ...
        <label
          htmlFor={color}
          className={`w-8 h-8 flex items-center justify-center rounded-full cursor-pointer border border-gray-300 ${
            selectedColors.includes(color) ? 'bg-gray-800' : 'bg-white'
          }`}
        >
          <span
            className={`w-6 h-6 rounded-full ${
              selectedColors.includes(color) ? 'bg-gray-800' : color.toLowerCase()
            }`}
          />
        </label>
        <p className="text-xs text-center mt-1">{color}</p> {/* Add this line for color name */}
      </div>
    ))}
  </div>
</div>
